...
class MainWindow(tk.Frame):
    def __init__(self, master):
        ...
        self.str_var.set('Digite algo e pressione Enter')
        entry = tk.Entry(
            master=self,
        ...
        entry.bind(
            sequence='<Key-Return>',
            func=self.on_key_return_pressed,
        )
        entry.pack(expand=True, fill=tk.X)

    def on_key_return_pressed(self, event):
        print(f'Evento: {event}')
        print(f'Valor: {self.str_var.get()}')


class Application(tk.Tk):
    def __init__(self):
        super().__init__()
        self.wm_title('Python e Tkinter: tk.Entry().')
